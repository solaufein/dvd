        ...
        <div id="myform">
            ...
            <div class="inputs">
                <ul>
                    ...
                    <li><label> <spring:message code="clients.clientsList.pesel"/>: </label> <c:out
                            value="${client.pesel}"/></li>
                    <li><label> <spring:message code="clients.clientsList.city"/>: </label> <c:out
                            value="${client.city}"/></li>
                    <li><label> <spring:message code="clients.clientsList.street"/>: </label> <c:out
                            value="${client.street}"/></li>
                    <li><label> <spring:message code="clients.clientsList.phoneNumber"/>: </label> <c:out
                            value="${client.phoneNumber}"/></li>
                    <li><label> <spring:message code="clients.clientsList.email"/>: </label> <c:out
                            value="${client.email}"/></li>
                </ul>
            </div>
        </div>
        ...
        <div class="table">
            <table>
                <tr>
                    ...
                    <td>
                        <spring:message code="clients.clientHistoryList.expectedReturnDate"/>
                    </td>
                    <c:if test="${not empty details.payDate}">
                        <td></td>
                    </c:if>
                    <c:if test="${empty details.payDate}">
                        <td></td>
                    </c:if>
                </tr>
                <c:forEach items="${clientDetails}" var="details">
                    <tr>
                        ...
                        <c:if test="${empty details.payDate}">
                            <td>
                                <form name="returnMovie" action=" <c:url value="/emp/return/movie"/>" method="post">
                                    <input type="hidden" name="movieCopyId" value="${details.mcId}"/>
                                    <input type="hidden" name="clientId" value="${client.id}"/>
                                    <input type="hidden" name="registryId" value="${details.id}"/>
